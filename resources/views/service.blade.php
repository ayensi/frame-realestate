...
<header class="page-header" data-background="{{asset('storage/'.$services[0]->header_image)}}" data-stellar-background-ratio="1.15">
    <div class="container">
        <h1 style="display: block; font-size: 30px; margin-left: 50px;">{{\Illuminate\Support\Facades\App::getLocale() == 'tr' ? 'Hizmetlerimiz' : 'Services'}}</h1>
        <ol class="breadcrumb">
            <li class="breadcrumb-item"><a href="#">{{\Illuminate\Support\Facades\App::getLocale() == 'tr' ? 'Anasayfa' : 'Home'}}</a></li>
            <li class="breadcrumb-item active" aria-current="page">{{\Illuminate\Support\Facades\App::getLocale() == 'tr' ? 'Hizmetlerimiz' : 'Service'}}</li>
        </ol>
    </div>
    <!-- end container -->
</header>
        ...
        <div class="row">
            ...
            <div class="col-lg-4 col-md-6 wow fadeInUp" data-wow-delay="0s">
                <figure data-stellar-ratio="1.07"> <a href= data-fancybox><img
                            src="{{asset('storage/'.$service->services_image)}}" alt="Image"></a>
                    <figcaption>
